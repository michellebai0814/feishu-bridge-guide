...
如果你准备把这套东西发到 GitHub，推荐公开保留这些内容：
...
这样会更像一个“可直接复用的 Skill 包”，而不是一堆本地调试材料混在一起。

其中：

- `.env.example` 是配置模板（照着填的示例文件），不是你的真实密钥。
- 真正发布时，只公开 `.env.example`，不要把你自己实际在用的 `.env` 一起传上去。
...
| 你现在在做什么 | 大概率属于哪一类 | 你通常会在哪里操作 |
| --- | --- | --- |
| 跟 bot（机器人）说话、发口令（命令） | 飞书聊天页 | 飞书群聊、私聊 |
| 开权限、收消息、接按钮点击 | 飞书开放平台后台 | 飞书应用后台 |
| 打开某个功能开关 | 本地配置文件 | `config.toml`（配置文件） |
| 让它读取你的资料、输出文件 | 本地项目目录 | 你的电脑文件夹 |
...
| 词 | 大白话解释 | 典型例子 | 优先去哪里看 |
| --- | --- | --- | --- |
| 口令（命令） | 你在聊天里直接发的一句指令 | `/help`、`/new`、`/web setup` | 飞书聊天页 |
| 权限 | 飞书允不允许这个应用做某件事 | 读群成员、发消息 | 飞书开放平台后台 |
| 事件 | 飞书把“发生了什么”通知给你 | 收到新消息 | 飞书开放平台后台 |
| 回调 | 飞书把“用户点了什么”通知给你 | 卡片按钮点击 | 飞书开放平台后台 |
| 配置项 | 你本地开的功能开关 | `resolve_mentions = true` | `config.toml` |
| 工作目录 | agent（智能代理）当前正在看的文件夹 | `/dir` 切目录 | 飞书聊天页 + 本地项目目录 |
